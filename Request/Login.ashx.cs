...
namespace Tank.Request
{
    ...
    public class Login : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
                ...
                if (!string.IsNullOrEmpty(p))
                {
                    ...
                    if (strList.Length == 4)
                    {
                        ...
                        if (true)
                        {
                            ...
                            PlayerInfo player = inter.CreateLogin(name, newPwd, 0, ref message, ref isFirst, IP, ref isError, firstValidate, ref isActive, site, nickname);
            ...
            {
                byte[] total = Convert.FromBase64String(p);
                log.Error("User Login error: " + "(--"+ StaticFunction.RsaCryptor.KeySize + "--)" + ex.ToString());
                log.Error("--dataarray: " + Marshal.ToHexDump("fuckingbitch " + total.Length, total));
                value = false;
