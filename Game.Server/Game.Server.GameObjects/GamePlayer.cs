        ...
        public void ClearStoreBag()
        {
            ...
            for (int i = 0; i < StoreBag.Capalility; i++)
            {
                ...
                if (itemAt != null)
                {
                    ...
                    {
                        if (PropBag.StackItemToAnother(itemAt) || PropBag.AddItem(itemAt))
                        {
                            StoreBag.TakeOutItem(itemAt);
                        }
                    }
                    else if (itemAt.Template.BagType == eBageType.EquipBag)
                    {
                        int place = EquipBag.FindFirstEmptySlot(31);
                        if (place > 0)
                        {
                            if (EquipBag.StackItemToAnother(itemAt) || EquipBag.AddItemTo(itemAt, place))
                            {
                                StoreBag.TakeOutItem(itemAt);
                            }
                        }
                    ...
                    if (LoadFromDatabase())
                    {
                        ...
                        if (PlayerCharacter.Repute > 0 && PlayerCharacter.Repute <= 10)
                        {
                            ...
                            if (Ranked == null | Ranked.Length < 1)
                                Ranked = "Người Chơi";
                            string NoticeOnline = string.Format("{1} [{2}] lực chiến {3} hạng {4} đã online!", ZoneName, Ranked, PlayerCharacter.NickName, PlayerCharacter.FightPower, PlayerCharacter.Repute);
