			...
			try
			{
				...
				while (resultDataReader.Read())
				{
					ShopItemInfo item = new ShopItemInfo
					{
						...
						Sort = resultDataReader["Sort"] == DBNull.Value ? 0 : int.Parse(resultDataReader["Sort"].ToString()),
						IsVouch = resultDataReader["IsVouch"] == DBNull.Value ? 0 : int.Parse(resultDataReader["IsVouch"].ToString()),
